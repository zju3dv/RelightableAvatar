...
class FrameSampler(Sampler):
    ...
    def __init__(self, dataset):
        ...
        inds = inds.reshape(ni, -1)[::cfg.test.frame_sampler_interval, ::cfg.test.view_sampler_interval][cfg.begin_ith_latent:]
...
class MeshFrameSampler(FrameSampler):
    """Sampler certain frames for test, including a -1 frame for original mesh
    """

    def __init__(self, dataset):
        super(MeshFrameSampler, self).__init__(dataset)
        if cfg.vis_can_mesh:
            self.inds = [-1]
        elif cfg.vis_tpose_mesh and cfg.track_tpose_mesh:
            self.inds = np.concatenate([[-1], self.inds])
