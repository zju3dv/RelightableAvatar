...
def add_drive_function(matrix_dict: dict[list, np.ndarray], posedirs: np.ndarray, bonedirs: np.ndarray, blend_poses: np.ndarray, blend_bones: np.ndarray, use_bones: bool = True, use_parent: bool = True, grp_idx: int = 0):
    ...
    def driver_interpolate(
        ...
        return weight  # FIXME: wasted computation
...
# def main():
#     BASE_DIR = "c:/users/56295/desktop/mesh/deformation"
#     DATA_PATH = f"{BASE_DIR}/ani_313_blvd_obgr_noinit_pose_300/-1/mesh.npz"

#     log(f"loading data from: {DATA_PATH}")
#     item = np.load(DATA_PATH, allow_pickle=True)
#     item = {**item}

#     vertices = item['vertices'].tolist()  # blender doesn't accept np array...
#     triangles = item['triangles'].tolist()
#     mesh = add_mesh('anisdf', vertices, triangles)

#     parents = item['parents']
#     location = item['big_joints']
#     weights = item['weights']
#     weights /= np.linalg.norm(weights)
#     bone_ids = item['bones']
#     joints = add_joints('anijoints', mesh, location, parents)
#     assign_bw(mesh, weights, bone_ids)

#     pose_bones = joints.pose.bones
#     pin = pose_bones['pin']
#     fix_transform(0, pin)
#     all_poses, all_rs, all_ts = item['all_poses'], item['all_rs'], item['all_ts']
#     indices = list(range(len(all_poses)))
#     for frame_id, (poses, Rh, Th) in enumerate(tqdm(zip(all_poses, all_rs, all_ts))):
#         insert_keyframe(frame_id, poses, Rh, Th, pose_bones)
...
#     deforms = item['deforms']
#     def_idx = item['def_idx']
#     for d, idx in tqdm(zip(deforms, def_idx)):
#         sk = mesh.shape_key_add(name=f'def {idx}')
#         sk.interpolation = 'KEY_LINEAR'
#         for i in range(len(vertices)):
#             sk.data[i].co = vertices[i] + d[i]  # assign vertex location

#     for i, idx in enumerate(tqdm(def_idx)):
#         if i > 0:
#             mesh.data.shape_keys.key_blocks[f'def {def_idx[i-1]}'].value = 0
#             mesh.data.shape_keys.key_blocks[f'def {def_idx[i-1]}'].keyframe_insert('value', frame=idx)

#         if i < len(def_idx)-1:
#             mesh.data.shape_keys.key_blocks[f'def {def_idx[i+1]}'].value = 0
#             mesh.data.shape_keys.key_blocks[f'def {def_idx[i+1]}'].keyframe_insert('value', frame=idx)

#         mesh.data.shape_keys.key_blocks[f'def {idx}'].value = 1
#         mesh.data.shape_keys.key_blocks[f'def {idx}'].keyframe_insert('value', frame=idx)
...
# POSE_NAME = 'anijoints'
# SHAPE_KEY_NAME = 'Key'
# POSE_ACTION_NAME = 'anijointsAction'
# SHAPE_KEY_ACTION_NAME = 'KeyAction'
# SHAPE_KEY_NAME_PREFIX = 'def '
# PLACEHOLDER_DATA_DIM = 10 # 4 axis angle, 3 xyz location, 3 scale for 1 fcurve
# PLACEHOLDER_CNT = 1
# NUM_BONES = 25
# ANGLE_AXIS_DATA_DIM = 4
# ANGLE_AXIS_POSE_DIM = 3

# # name conversion
# frame_index_to_shape_key_name = lambda x: f"{SHAPE_KEY_NAME_PREFIX}{x}"
# shape_key_name_to_frame_index = lambda x: int(x[len(SHAPE_KEY_NAME_PREFIX):])

# # shortcuts
# shape_key_blocks = bpy.data.shape_keys[SHAPE_KEY_NAME].key_blocks
# fcurves = bpy.data.actions['anijointsAction'].fcurves

# num_shape_keys = len(shape_key_blocks)


# def get_frame_poses(frame_index):
#     poses = np.zeros((NUM_BONES, ANGLE_AXIS_DATA_DIM))
#     for i in range(NUM_BONES):
#         for j in range(ANGLE_AXIS_DATA_DIM):
#             fcu_index = PLACEHOLDER_CNT * PLACEHOLDER_DATA_DIM + i * ANGLE_AXIS_DATA_DIM + j
#             value = fcurves[fcu_index].keyframe_points[frame_index].co[-1]
#             poses[i, j] = value
# #    poses = poses[..., 1:] * poses[..., :1]
# #    poses = poses[..., 1:]
#     return poses # n_bones, n_dim

# all_poses = {}
# #all_poses = []
# for i, shape_key in enumerate(shape_key_blocks):
#     if shape_key.name == "basis": continue
#     frame_index = shape_key_name_to_frame_index(shape_key.name)
#     all_poses[shape_key.name] = get_frame_poses(frame_index)

# #all_poses = np.stack(all_poses) # (n_shape_keys, n_bones, n_dim)

# def get_current_poses(pose_bones):
#     # FIXME: this is dependent on the order in which you added in the pose in the keyframe animation
#     pose_bones = sorted(pose_bones[PLACEHOLDER_CNT+1:], key=lambda x: int(x.name)) + \
#     [pose_bones[PLACEHOLDER_CNT]]
#     poses = np.stack([np.array(bone.rotation_axis_angle) for bone in pose_bones])
# #    poses = poses[..., 1:] * poses[..., :1]
# #    poses = poses[..., 1:]
#     return poses # n_bones, n_dim

# def pca_interpolate(pose_bones):
#     # whatever the input value, we'll use the global pose parameters instead
#     # 0. load available shape key and pose values (load only the intersection of these sets)
#     # 1. project the pose to those defined in shape key
#     # 2. do interpolation based on projected value -> new deformation

#     # note that in extract.py we named shape key as: "def {idx}", and we named the keys: Key
#     # this idx corresponds to the actual poses' key frame animation
#     total = 0
#     values = np.zeros(len(shape_key_blocks))
#     cur_poses = get_current_poses(pose_bones).ravel()
#     for i, shape_key in enumerate(shape_key_blocks[1:]):
#         shape_poses = all_poses[shape_key.name].ravel()
#         proj = shape_poses.dot(cur_poses) / np.linalg.norm(shape_poses) ** 2
#         values[i] = proj
#         total += proj
# #    total = values.sum()
#     for i, shape_key in enumerate(shape_key_blocks):
#          shape_key.value = values[i] / total
# #    log(f'updating shape key activations: sum: {total}, min: {values.min()}, max: {values.max()}')
#     log(f'updating shape key activations: sum: {total}')
#     return 0

# #    log(self.name)
# #    log(type(self)
# #    log(f"updating: {name}")
# #    shape_poses = all_poses[name].ravel()
# #    cur_poses = get_current_poses(pose_bones).ravel()
# #    proj = shape_poses.dot(cur_poses) / np.linalg.norm(shape_poses) ** 2
...
# bpy.app.driver_namespace['pca_interpolate'] = pca_interpolate    # add function to driver_namespace

# joints = bpy.data.objects[POSE_NAME]
# anisdf = bpy.data.objects['anisdf']
# anisdf.driver_remove('location', 0)
# driver = anisdf.driver_add('location', 0).driver
# driver.type = 'SCRIPTED'
# driver.use_self = True
# driver.expression = 'pca_interpolate(pose_bones)'
# var = driver.variables.new()
# var.targets[0].id_type = 'OBJECT'
# var.targets[0].id = joints
...
# #    if shape_key.name == 'basis': continue # skip the basis shape key
# #    shape_key.driver_remove('value')
# #    driver = shape_key.driver_add('value').driver
# #    driver.type = 'SCRIPTED'
# #    driver.use_self = True
# #    driver.expression = 'pca_interpolate(pose_bones, self.name)'
# #    var = driver.variables.new()
# #    var.targets[0].id_type = 'OBJECT'
# #    var.targets[0].id = joints
# #    var.targets[0].data_path = "pose.bones"
