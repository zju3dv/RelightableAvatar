...
def colormap(v: torch.Tensor, cm=turbo):
    """
    ...
    *x*, *y0*, *y1* tuples.  In each sequence, *x* must increase
    ...
    if isinstance(cm, dict):
        ...
        for key in ['red', 'green', 'blue']:
            channel = torch.zeros_like(v)
            for i in range(len(cm[key]) - 1):
                b0 = cm[key][i][0]
                b1 = cm[key][i + 1][0]
                c0 = cm[key][i][2]
                c1 = cm[key][i + 1][1]
                ...
                channel += (c1 * interp + c0 * (1 - interp)) * valid
            color.append(channel)
        color = torch.cat(color, dim=-1)
    elif isinstance(cm, list) or isinstance(cm, tuple):
        cm = torch.from_numpy(np.array(cm)).to(v.device, non_blocking=True)  # N, 3
        color = v.new_zeros([*v.shape[:-1], 3])  # B, P, 3
        for i in range(len(cm) - 1):  # a loop...
            b0 = i / (len(cm) - 1)
            b1 = (i + 1) / (len(cm) - 1)
            c0 = cm[i]
            c1 = cm[i + 1]
        ...
        raise NotImplementedError(f'Unsupported colormap implementation for type {type(cm)}: {cm}')
    return color
