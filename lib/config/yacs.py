...
try:
    _FILE_TYPES = (file, io.IOBase)  # type: ignore
    _PY2 = True
except NameError:
...
if _PY2:
    _VALID_TYPES = _VALID_TYPES.union({unicode})  # type: ignore noqa: F821
...
class CN(CfgNode):
    def merge_strain(cfg, current_cfg: CfgNode):
        if 'parent_cfg' in current_cfg.keys():
            with open(current_cfg.parent_cfg, 'r') as f:
                p_cfg = load_cfg(f)
            cfg.merge_strain(p_cfg)  # recursion here
        if 'parent_cfgs' in current_cfg.keys():
            for parent in current_cfg.parent_cfgs:
                with open(parent, 'r') as f:
                    p_cfg = load_cfg(f)
                cfg.merge_strain(p_cfg)  # recursion here
        # extra consideration needed to avoid merging self (perfomance)
        cfg.merge_from_other_cfg(current_cfg)  # root config should be the last layer
...
# should also accept decendants of _VALID_TYPES and CfgNode
def _valid_type(value, allow_cfg_node=False):
    def _is_one_of(value, types):
        for t in types:
            if isinstance(value, t):
                return True
        return False
    return _is_one_of(value, _VALID_TYPES) or (allow_cfg_node and isinstance(value, CfgNode))
...
def _check_and_coerce_cfg_value_type(replacement, original, key, full_key):
    ...
    # Conditionally casts
    # list <-> tuple
    casts = [(tuple, list), (list, tuple), (int, float), (float, int), (CN, CfgNode), (CfgNode, CN)]
    # For py2: allow converting from str (bytes) to a unicode string
    try:
        casts.append((str, unicode))  # type: ignore noqa: F821
    except Exception:
        pass
...
def _load_module_from_file(name, filename):
    if _PY2:
        module = imp.load_source(name, filename) # type: ignore
    else:
